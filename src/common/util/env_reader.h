#ifndef __RFLOW_COMMON_UTIL_ENV_READER_H__
#define __RFLOW_COMMON_UTIL_ENV_READER_H__
...
inline int64_t ReadEnvIntOrDefault(const char* name, int64_t fallback, int64_t min_v, int64_t max_v) {
    const char* env_value = std::getenv(name);
    if (!env_value || !env_value[0]) return fallback;
    char* parse_end = nullptr;
    const long long parsed_value = std::strtoll(env_value, &parse_end, 10);
    if (parse_end == env_value || (parse_end && *parse_end != '\0')) return fallback;
    if (parsed_value < min_v || parsed_value > max_v) return fallback;
    return parsed_value;
}
...
using rflow::common::base::ReadEnvIntOrDefault;
using rflow::common::base::ReadEnvSizeInRange;
}  // namespace rflow::common::util

#endif  // __RFLOW_COMMON_UTIL_ENV_READER_H__
