...
rflow_err_t librflow_svc_stream_param_set_h264_profile(librflow_svc_stream_param_t p, const char* profile) {
    RFLOW_CHECK_HANDLE(p, rflow::service::kMagicStreamParam);
    if (const rflow_err_t rc = rflow::common::base::CopyRequiredTrimmedToken(
            profile, rflow::common::base::kStreamParamTokenMaxLen, &p->h264_profile);
        rc != RFLOW_OK) {
        return rc;
    }
...
rflow_err_t librflow_svc_stream_param_set_h264_level(librflow_svc_stream_param_t p, const char* level) {
    RFLOW_CHECK_HANDLE(p, rflow::service::kMagicStreamParam);
    if (const rflow_err_t rc = rflow::common::base::CopyRequiredTrimmedToken(
            level, rflow::common::base::kStreamParamTokenMaxLen, &p->h264_level);
        rc != RFLOW_OK) {
        return rc;
    }
...
rflow_err_t librflow_svc_stream_param_set_stun_server(librflow_svc_stream_param_t p, const char* stun_url) {
    RFLOW_CHECK_HANDLE(p, rflow::service::kMagicStreamParam);
    if (const rflow_err_t rc = rflow::common::base::CopyRequiredIceUrl(stun_url, &p->stun_server); rc != RFLOW_OK) {
        return rc;
    }
...
rflow_err_t librflow_svc_stream_param_set_turn_server(librflow_svc_stream_param_t p,
                                                      ...
                                                      const char* password) {
    RFLOW_CHECK_HANDLE(p, rflow::service::kMagicStreamParam);
    if (const rflow_err_t rc = rflow::common::base::CopyRequiredIceUrl(turn_url, &p->turn_server); rc != RFLOW_OK) {
        return rc;
    }
    if (const rflow_err_t rc = rflow::common::base::CopyOptionalIceCredential(username, &p->turn_username); rc != RFLOW_OK) {
        return rc;
    }
    if (const rflow_err_t rc = rflow::common::base::CopyOptionalIceCredential(password, &p->turn_password); rc != RFLOW_OK) {
        return rc;
    }
