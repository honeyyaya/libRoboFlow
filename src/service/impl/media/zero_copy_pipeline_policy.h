 ...
 *
 * 历史背景：camera_video_track_source.cpp 中曾分别用三个独立函数读三个环境变量来决定
 ...
 *   2. 单元测试：可以注入 fake env 直接验证 policy 字段；
...
///
/// 行为约定：
///   - WEBRTC_MJPEG_ZERO_COPY_TO_ENC 缺省 / 空串 / 非 '0' → prefer_native = true；
///     '0' → false。
///   - WEBRTC_MJPEG_V4L2_DMABUF 缺省 → use_v4l2_ext_dmabuf 取
///     v4l2_ext_dma_config_default；非空 → '0' = false / 其他 = true。
///   - WEBRTC_MJPEG_RGA_TO_MPP 同上规则；不带 LIBRGA 时强制 false。
///
/// 调用方应在 hot path 内每次 zero-copy 决策点重新求值（成本约 3 次 getenv），
/// 这与历史行为一致，方便运维 hot-reload。
