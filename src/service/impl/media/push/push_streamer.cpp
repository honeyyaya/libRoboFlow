...
#include "rtc/peer_connection_state_map.h"
...
public:
    ...
    void Shutdown() {
        ...
        if (receiver_) {
            ...
            receiver_ = nullptr;
            ClosePeerConnectionWithDeadline(recv, "PushStreamer", "loopback receiver", 8);
        }
        ...
        if (peer_connection_) {
            ...
            ClosePeerConnectionWithDeadline(sender, "PushStreamer", "publisher", 8);
        }
        for (auto& kv : peer_connections_) {
            if (kv.second) {
                ClosePeerConnectionWithDeadline(kv.second, "PushStreamer", "subscriber", 8);
            }
        }
    ...
    void ClosePeerForSubscriber(const std::string& peer_id) {
        ...
        if (pc) {
            ClosePeerConnectionWithDeadline(pc, "PushStreamer", "subscriber leave", 8);
        }
        observer.reset();
    ...
    void NotifyConnectionState(webrtc::PeerConnectionInterface::PeerConnectionState state) {
        ...
        on_connection_state_(rflow::core::rtc::MapPeerConnectionState(state));
    }
