#ifndef __RFLOW_CORE_SIGNAL_SHARED_SIGNALING_IO_MANAGER_H__
#define __RFLOW_CORE_SIGNAL_SHARED_SIGNALING_IO_MANAGER_H__
...
class SharedSignalingIoManager {
 public:
  bool RegisterSession(const std::shared_ptr<SlotT>& slot) {
  ...
  void UnregisterSession(const std::shared_ptr<SlotT>& slot) {
    if (!slot) return;

    OwnerT* owner = slot->owner.exchange(nullptr, std::memory_order_acq_rel);
 ...
 private:
  ...
  bool EnsureStarted() {
    ...
    int wake_pipe[2]{-1, -1};
    if (::pipe(wake_pipe) != 0) return false;
    ...
    io_thread_ = std::thread(&SharedSignalingIoManager::Run, this);
    started_ = true;
    return true;
  }
  ...
  std::vector<std::shared_ptr<SlotT>> SnapshotSessions() {
    std::vector<std::shared_ptr<SlotT>> out;
    std::lock_guard<std::mutex> lock(mu_);
    ...
    for (const auto& [fd, slot] : sessions_) {
      ...
      if (slot->fd.load(std::memory_order_acquire) < 0) continue;
      out.push_back(slot);
    }
    return out;
  }

  void DispatchLine(const std::shared_ptr<SlotT>& slot, std::string_view line) {
    rflow::signal::Message msg;
    if (!rflow::signal::ParseMessage(line, &msg)) return;

    OwnerT* owner = slot->owner.load(std::memory_order_acquire);
    if (!owner) return;
    owner->ParseAndDispatch(msg);
  }

  void CloseSession(const std::shared_ptr<SlotT>& slot, std::string_view error) {
    if (!slot) return;
  ...
  void HandleReadable(const std::shared_ptr<SlotT>& slot) {
    const int fd = slot->fd.load(std::memory_order_acquire);
    if (fd < 0 || !slot->active.load(std::memory_order_acquire)) return;

    constexpr size_t kReadChunkBytes = 64 * 1024;
    constexpr size_t kMaxBufferedBytes = 4 * 1024 * 1024;
    char tmp[kReadChunkBytes];
    const ssize_t n = ::recv(fd, tmp, sizeof(tmp), 0);
    if (n > 0) {
      slot->read_buffer.append(tmp, static_cast<size_t>(n));
      if (slot->read_buffer.size() > kMaxBufferedBytes) {
      ...
      if (slot->parse_offset > 0 &&
          (slot->parse_offset == slot->read_buffer.size() ||
           slot->parse_offset >= (slot->read_buffer.size() / 2))) {
  ...
  void Run() {
    while (running_.load(std::memory_order_acquire)) {
      ...
      const int ready = ::poll(poll_fds.data(), static_cast<nfds_t>(poll_fds.size()), -1);
      if (ready < 0) {
  ...
  std::mutex mu_;
  std::unordered_map<int, std::shared_ptr<SlotT>> sessions_;
};

}  // namespace rflow::core::signal

#endif  // __RFLOW_CORE_SIGNAL_SHARED_SIGNALING_IO_MANAGER_H__
