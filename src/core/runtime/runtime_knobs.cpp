...
constexpr KnobSpec kKnobTable[] = {
    ...
    {"RFLOW_MEDIA_THREAD_NICE", nullptr, KnobGroup::kMediaCapture, KnobKind::kInt, -10, -20, 19, "",
     "Process nice value when SCHED=nice or auto high-fps tune."},
    {"RFLOW_MEDIA_THREAD_AUTO", nullptr, KnobGroup::kMediaCapture, KnobKind::kBool, 1, 0, 1, "",
     "Auto-raise V4L2/MJPEG thread nice for >=45fps when RFLOW_MEDIA_THREAD_SCHED unset."},
    ...
    {"RFLOW_V4L2_BUFFER_COUNT", nullptr, KnobGroup::kMediaMjpeg, KnobKind::kInt, 0, 2, 32, "",
     "Override V4L2 mmap buffer count after fps preset (0 = use preset)."},
    {"RFLOW_MJPEG_QUEUE_MAX", nullptr, KnobGroup::kMediaMjpeg, KnobKind::kInt, 0, 1, 32, "",
     "Override MJPEG decode queue depth after fps preset (0 = use preset; clamped to v4l2_bufs-1)."},
    {"RFLOW_NV12_POOL_SLOTS", nullptr, KnobGroup::kMediaMjpeg, KnobKind::kInt, 0, 4, 16, "",
     "Override NV12 pool slots after fps preset (0 = use preset)."},
    ...
    {"RFLOW_MJPEG_ZERO_COPY_TO_ENC", nullptr, KnobGroup::kMediaMjpeg, KnobKind::kBool, 1, 0, 1, "",
     "Prefer kNative MPP dec buffer pass-through to H264 encoder (default on)."},
    {"RFLOW_MJPEG_V4L2_EXT_DMA", nullptr, KnobGroup::kMediaMjpeg, KnobKind::kBool, 1, 0, 1, "",
     "Import V4L2 MJPEG EXPBUF directly into MPP decoder; 0 restores memcpy input."},
    {"RFLOW_MPP_ENC_MPP_BUFFER_DIRECT", nullptr, KnobGroup::kMppEncoder, KnobKind::kBool, 1, 0, 1, "",
     "Hold non-split Annex-B MPP output buffers directly in WebRTC encoded frames."},
    {"RFLOW_MPP_ENC_DIRECT_MAX_OUTSTANDING", nullptr, KnobGroup::kMppEncoder, KnobKind::kInt, 16, 1, 128, "",
     "Maximum MPP output buffers retained by WebRTC before assembly fallback."},
    ...
    {"RFLOW_RGA_SCALE_POOL_SLOTS", nullptr, KnobGroup::kMediaMjpeg, KnobKind::kInt, 8, 2, 32, "",
     "DRM buffer pool slots for RGA NV12 weak-network scale output."},
    {"RFLOW_RGA_SCALE_TRACE", nullptr, KnobGroup::kMediaMjpeg, KnobKind::kBool, 1, 0, 1, "",
     "Log RGA NV12 crop/scale (WebRTC adaptation path)."},
    {"RFLOW_RGA_SCALE_TRACE_EVERY_N", nullptr, KnobGroup::kMediaMjpeg, KnobKind::kInt, 60, 1, 600, "",
     "When RGA scale trace is on, log at most one ok line per N scaled frames (failures always logged)."},
    ...
    {"RFLOW_MPP_ENC_RC_APPLY_MIN_MS", nullptr, KnobGroup::kMppEncoder, KnobKind::kInt, 200, 0, 5000, "",
     "Min interval between runtime MPP RC (SET_CFG) updates from SetRates; 0 disables throttle."},
    {"RFLOW_MPP_ENC_RC_APPLY_MIN_BPS_DELTA", nullptr, KnobGroup::kMppEncoder, KnobKind::kInt, 50000, 1000, 500000,
     "",
     "Min target bitrate change (bps) before pushing RC update to MPP."},
    {"RFLOW_MPP_ENC_RC_APPLY_BPS_DELTA_PCT", nullptr, KnobGroup::kMppEncoder, KnobKind::kInt, 5, 1, 50, "",
     "Min target bitrate change (percent) before pushing RC update to MPP."},
    ...
    {"RFLOW_MPP_ENC_NATIVE_ZERO_COPY", nullptr, KnobGroup::kMppEncoder, KnobKind::kBool, 1, 0, 1, "",
     "Pass MPP decoder DRM buffer to encoder (direct inc_ref)."},
    {"RFLOW_MPP_ENC_SUPPORTS_NATIVE_HANDLE", nullptr, KnobGroup::kMppEncoder, KnobKind::kBool, 0, 0, 1, "",
     "EncoderInfo.supports_native_handle; default off so WebRTC can CropAndScale on weak network."},
    {"RFLOW_MPP_ENC_QP_SCALE_LOW", nullptr, KnobGroup::kMppEncoder, KnobKind::kInt, 24, 0, 51, "",
     "QualityScaler QP low threshold (scale up when avg QP below)."},
    {"RFLOW_MPP_ENC_QP_SCALE_HIGH", nullptr, KnobGroup::kMppEncoder, KnobKind::kInt, 36, 1, 51, "",
     "QualityScaler QP high threshold (scale down when avg QP above). Default 36 (was 37)."},
    {"RFLOW_MPP_ENC_QP_TRUSTED", nullptr, KnobGroup::kMppEncoder, KnobKind::kBool, 0, 0, 1, "",
     "EncoderInfo.is_qp_trusted; default 0 = BandwidthQualityScaler for weak-net resolution downscale."},
    {"RFLOW_ADAPT_MIN_PIXELS", nullptr, KnobGroup::kMediaMjpeg, KnobKind::kInt, 640 * 360, 320 * 180, 1280 * 720,
     "",
     "Weak-net resolution floor (pixels); e.g. 640x360=230400 prevents stepping below ~640p."},
    {"RFLOW_DEBUG_FORCE_CAPTURE_SCALE_W", nullptr, KnobGroup::kMediaMjpeg, KnobKind::kInt, 0, 0, 4096, "",
     "Debug: force RGA/CropAndScale capture to this width before WebRTC (0=off)."},
    {"RFLOW_DEBUG_FORCE_CAPTURE_SCALE_H", nullptr, KnobGroup::kMediaMjpeg, KnobKind::kInt, 0, 0, 4096, "",
     "Debug: force capture scale height (pair with RFLOW_DEBUG_FORCE_CAPTURE_SCALE_W)."},
    {"RFLOW_DEBUG_FORCE_CAPTURE_SCALE_AFTER_N", nullptr, KnobGroup::kMediaMjpeg, KnobKind::kInt, 0, 0, 1000000, "",
     "Debug: start force capture scale after N frames (0=immediate; use ~120 to test MPP reconfigure)."},
    ...
    {"RFLOW_MPP_ENC_ZERO_COPY_BIND", nullptr, KnobGroup::kMppEncoder, KnobKind::kString, 0, 0, 0, "direct",
     "Zero-copy dec→enc bind: direct (inc_ref) | null (mpp import misc group) | external (legacy EXTERNAL group)."},
    {"RFLOW_MPP_ENC_IMPORT_POOL_COUNT", nullptr, KnobGroup::kMppEncoder, KnobKind::kInt, 0, 0, 64, "",
     "When IMPORT_POOL_LIMIT=1: EXTERNAL import slot count (0=auto from v4l2+mjpeg_q+slack)."},
    {"RFLOW_MPP_ENC_IMPORT_POOL_LIMIT", nullptr, KnobGroup::kMppEncoder, KnobKind::kBool, 0, 0, 1, "",
     "Apply mpp_buffer_group_limit_config on EXTERNAL import group (default off; breaks dynamic import_with_tag)."},
    {"RFLOW_MPP_ENC_IMPORT_POOL_SLACK", nullptr, KnobGroup::kMppEncoder, KnobKind::kInt, 2, 0, 16, "",
     "Extra import slots beyond MJPEG dec_out_pool for WebRTC encode/send in-flight frames."},
    {"RFLOW_MJPEG_DEC_OUT_POOL_SLACK", nullptr, KnobGroup::kMppEncoder, KnobKind::kInt, 2, 0, 8, "",
     "Slack added to v4l2+mjpeg_q when auto-sizing dec/import pools (matches camera StartDirectV4l2)."},
    {"RFLOW_MPP_ENC_IMPORT_RESET_LOOP_PAUSE", nullptr, KnobGroup::kMppEncoder, KnobKind::kInt, 5, 2, 100, "",
     "After this many import group resets without recovery, pause zero-copy import (CPU copy)."},
    {"RFLOW_MPP_ENC_IMPORT_RESET_FAILS", nullptr, KnobGroup::kMppEncoder, KnobKind::kInt, 3, 1, 100, "",
     "Recreate EXTERNAL import group after N consecutive import failures."},
    {"RFLOW_MPP_ENC_IMPORT_PAUSE_FAILS", nullptr, KnobGroup::kMppEncoder, KnobKind::kInt, 30, 3, 1000, "",
     "Skip import attempts after N consecutive failures until periodic probe."},
    {"RFLOW_MPP_ENC_IMPORT_PAUSE_PROBE_FRAMES", nullptr, KnobGroup::kMppEncoder, KnobKind::kInt, 120, 30, 6000, "",
     "While import paused, retry import group reset every N frames."},
