#include "rtc/factory/rtc_factory_common.h"

#include "base/global_config_ops.h"
#include "rtc/ice/rtc_ice_config.h"
...
void ApplySdkGlobalConfig(librflow_global_config_s& dst, const librflow_global_config_s& src) {
    rflow::common::base::CopyGlobalConfig(dst, src);
    const bool flexfec_explicit = (dst.flexfec != RFLOW_GLOBAL_FLEXFEC_DEFAULT);
    const bool flexfec_enabled  = (dst.flexfec == RFLOW_GLOBAL_FLEXFEC_ON);
    NotifyFlexfecTrialFromSdkConfig(flexfec_explicit, flexfec_enabled);
    if (dst.has_ice_ignore_interfaces) {
        NotifyIceIgnoreInterfacesFromSdkConfig(dst.ice_ignore_interfaces.c_str());
    } else {
        NotifyIceIgnoreInterfacesFromSdkConfig(nullptr);
    }
    rflow::common::base::ApplyLogConfigIfPresent(dst);
}

void ResetSdkGlobalConfigOverrides() {
    ResetFlexfecTrialSdkOverride();
    ResetIceIgnoreInterfacesSdkOverride();
}
