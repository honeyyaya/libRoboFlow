...
/// ??? RGA ???????????????????????/???????????????????????????????????????????????? 16???static int RgaY400MaxAspectRatio() {
    static int cached = -1;
    if (cached >= 0) {
        return cached;
    }
...
/// ??RK_FORMAT_YCbCr_400 ?????? RGA imcopy ?????? w??h??????? 1 ??????????? need<=w*h<=cap???///
/// ?????????????? w???h **????** MJPEG ?????????????????????????????? 1280??720???JPEG ????????????/// ???????**jpeg_len ??????* ???????????????????**????????** ??????????????????? RGA ??????????????????/// ???need ??????????????(w,h) ??????????????????????w ?????????????????????????**4096??64** ?????????
/// ?????????BSP ??RGA_BLIT ??? EINVAL??????????????????????**?????????????? 1** ?????????????????WEBRTC_MJPEG_RGA_MAX_ASPECT ????????static bool RgaPickY400Rect(size_t cap, size_t need, int* out_w, int* out_h) {
    ...
    for (int w = kMinW; w <= w_hi; w += kAlignW) {
        ...
        // ?????????????tmx/tmn??tmx/tmn < bmx/bmn ???????? tmx*bmn < bmx*tmn??????????????        const auto lhs = static_cast<unsigned long long>(tmx) * static_cast<unsigned long long>(bmn);
...
/// ????????? cap ??????????????? need ??????????????w??h ????????????????????? V4L2 dma ??????????MPP input ?????????????static size_t RgaY400SmallestTotalBytes(size_t need) {
    int w = 0;
    int h = 0;
...
/// UVC vb2 dma-buf ???MPP DRM dma-buf ???RGA ???????///
/// ???????????????MJPEG ????????????????????/// 1) **??????????? buffer ???????????RGA ???? fd**??`wrapbuffer_fd` ?? w/h/format?????????????RGA ??????????vb2/DRM gem ???????????????????
///    ????????????????**importbuffer_fd(fd, size_bytes)**??? librga im2d.hpp????? **wrapbuffer_handle**???/// 2) **?????????**????????BSP ??? RGA ?? **4G ??? DMA**??dmesg ?????????? RGA_MMU / >4G??????????????????????????dma32 / CMA heap???????????????????????????/// 3) **????heap**??UVC ??MPP ???????????????????????????????????????????memcpy ???????dma-heap ????????///
/// ????????????**import + wrapbuffer_handle**????????????? **wrapbuffer_fd**?????????????????static bool RgaCopyDmaBufJpeg(int src_fd,
                              size_t src_cap,
                              int dst_fd,
                              size_t dst_cap,
                              size_t jpeg_len) {
...
/// MPP ???NV12/NV21????????WebRTC NV12Buffer??????????????????????stride ?????????static bool CopyMppSemiPlanarToNv12(RK_U32 fmt,
                                    const uint8_t* src_y,
...
bool RkMppMjpegDecoder::Init() {
    ...
    // gstmppdec.c set_format??mpp_init ?????????allocator ???group
...
void* RkMppMjpegDecoder::PollMppFrame(int timeout_ms) {
    ...
    // ??gstmppjpegdec poll_mpp_frame ?????????0 ?????????????
