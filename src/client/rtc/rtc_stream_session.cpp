#include "rtc/rtc_stream_session.h"
...
class RtcStreamSession::PeerConnectionObserverImpl : public webrtc::PeerConnectionObserver {
 public:
    ...
    void OnTrack(webrtc::scoped_refptr<webrtc::RtpTransceiverInterface> transceiver) override {
        ...
        if (!receiver) return;
        const double floor_s = rflow::core::rtc::ReceiverVideoJitterMinDelaySeconds();
        rflow::core::rtc::ApplyVideoReceiverJitterMinDelayMs(
            receiver.get(), static_cast<int>(floor_s * 1000.0 + 0.5));
...
RtcStreamSession::RtcStreamSession(
    ...
    watchdog_enabled_ = rflow::common::util::ReadEnvBool("RFLOW_RECEIVER_KEYFRAME_WATCHDOG", true);
    watchdog_stuck_threshold_ms_ =
        rflow::common::util::ReadEnvIntOrDefault("RFLOW_RECEIVER_WATCHDOG_STUCK_MS", 300, 50, 5000);
    watchdog_cooldown_ms_ =
        rflow::common::util::ReadEnvIntOrDefault("RFLOW_RECEIVER_WATCHDOG_COOLDOWN_MS", 600, 100, 10000);
}
...
bool RtcStreamSession::CollectStats(librflow_stream_stats_s* out_stats) {
    ...
    rflow::core::rtc::InboundVideoCollectSnapshot snap;
    if (!rflow::core::rtc::SyncCollectInboundVideoSnapshot(peer_connection_.get(), &snap)) {
        return false;
    }

    const uint32_t jitter_min_delay_ms = static_cast<uint32_t>(
        jitter_min_delay_seconds_.load(std::memory_order_relaxed) * 1000.0 + 0.5);
    rflow::core::rtc::FillInboundVideoStreamStats(out_stats, snap.agg, snap.rtt_ms,
                                                   jitter_min_delay_ms);

    {
        const int64_t now_mono_ms = rflow::common::util::MonoTimeMs();
        std::lock_guard<std::mutex> blk(stats_bitrate_mu_);
        out_stats->bitrate_kbps = inbound_bitrate_.Update(
            snap.agg.bytes_received, now_mono_ms, snap.available_incoming_kbps);
    }
    return true;
}
...
void RtcStreamSession::StartWatchdogThread() {
    ...
    last_decode_progress_mono_ms_ = rflow::common::util::MonoTimeMs();
...
void RtcStreamSession::OnWatchdogStats(
    ...
    const int64_t now_ms       = rflow::common::util::MonoTimeMs();
