 ...
 *   - 处理 Client 侧的业务服务请求：on_service_req + librobrt_svc_service_reply（Client 侧应答为 librobrt_reply_to_service_req）；
 ...
 *     create_stream -> start_stream -> [push_video_frame]* -> stop_stream -> destroy_stream
 *   -> disconnect -> uninit
...
/**
 * 下行流统计（SDK 周期上报）；stats 仅回调栈内有效。
 * 不显式传 handle — 表示为本路 create_stream 所绑定 stream_cb 对应之流。
 */
typedef void (*librobrt_svc_on_stream_stats_fn)(librobrt_stream_stats_t stats,
                                                void *userdata);
