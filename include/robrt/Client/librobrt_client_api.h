 ...
 *
 * Client 角色：订阅远端 Service 的媒体流，接收解码前/后视频与音频数据。
 *   - 仅负责"拉"，不做采集/编码；
 *   - 编码/分辨率/帧率等参数由 Service 侧决定，Client 只能给出 hint；
 *   - 通过 on_video_frame / on_audio_frame 把帧回调给业务层。
 *
...
LIBROBRT_API_EXPORT robrt_err_t librobrt_connect_cb_set_on_stream_stats(librobrt_connect_cb_t cb, librobrt_on_stream_stats_fn fn);
...
LIBROBRT_API_EXPORT robrt_err_t librobrt_stream_param_set_enable_audio      (librobrt_stream_param_t p, bool enable);
...
/** 音频帧到达；frame 仅回调栈内有效 */
typedef void (*librobrt_on_audio_frame_fn)(librobrt_stream_handle_t handle,
...
LIBROBRT_API_EXPORT robrt_err_t librobrt_stream_cb_set_on_audio (librobrt_stream_cb_t cb, librobrt_on_audio_frame_fn fn);
 ...
 * @param cb          流回调（video/audio/state）
...
/** 主动请求关键帧（丢帧后快速恢复） */
LIBROBRT_API_EXPORT robrt_err_t librobrt_stream_request_keyframe(librobrt_stream_handle_t handle);
